...
def dashboard_cargas(request):
    ...
    tecnicos_base = TecnicoPuesto.objects.select_related('user', 'puesto').filter(disponible=True)
    tecnicos = [t for t in tecnicos_base if t.user]
    puestos = PuestoTrabajo.objects.all()
    all_users = User.objects.filter(is_active=True).exclude(id__in=[t.user_id for t in tecnicos]).order_by('username')
    ...
    ots = OrdenTrabajo.objects.filter(tecnico__isnull=False, inicio_programado__gte=q_start, inicio_programado__lte=q_end).select_related('rutina', 'aviso', 'ubicacion')
    cm = collections.defaultdict(lambda: {'horas': 0.0, 'ots': []})
    
    for ot in ots:
        anio, sem, _ = ot.inicio_programado.isocalendar()
        key = f"{anio}-{sem}"
        dur = (ot.fin_programado - ot.inicio_programado).total_seconds() / 3600
        cm[(ot.tecnico_id, key)]['horas'] += float(dur)
        cm[(ot.tecnico_id, key)]['ots'].append({
            'id': ot.id, 
            'nombre': ot.rutina.nombre if ot.rutina else (ot.aviso.descripcion[:30] if ot.aviso else "OT"), 
    ...
    for t in tecnicos:
        st = []
        for s in semanas:
            d = cm.get((t.user_id, s['key']), {'horas': 0.0, 'ots': []})
            hrs = d['horas']
        ...
        puesto_name = t.puesto.nombre if t.puesto else "Sin Puesto"
        td.append({'id': t.user_id, 'nombre': user_name, 'puesto': puesto_name, 'semanas': st})
        
    pd = []
    for p in puestos:
        pt = [t for t in tecnicos if t.puesto_id == p.id]
        if not pt: continue
        ct = sum(float(t.horas_semanales_max) for t in pt)
        sp = []
        for s in semanas:
            hp = sum(cm.get((t.user_id, s['key']), {'horas': 0.0})['horas'] for t in pt)
            pct = (hp/ct*100) if ct > 0 else 0
    ...
    return render(request, 'mantenimiento/dashboard_cargas.html', {
        ...
        'puestos_data': pd,
        'puestos': puestos,
        'usuarios_sin_puesto': all_users
    })

@staff_member_required
def asignar_puesto_ajax(request):
    if request.method == 'POST':
        ...
        try:
            ...
            horas = data.get('horas', 40.0)
            
            user = User.objects.get(id=user_id)
            puesto = PuestoTrabajo.objects.get(id=puesto_id)
            
            TecnicoPuesto.objects.update_or_create(
                user=user,
                defaults={'puesto': puesto, 'horas_semanales_max': horas, 'disponible': True}
            )
