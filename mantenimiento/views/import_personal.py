...
from ..tasks import import_personal_task
from django.views.decorators.csrf import csrf_exempt

@staff_member_required
def import_personal_background(request):
    """Renders the upload form for background import."""
    from django.contrib import admin
    context = {
        **admin.site.each_context(request),
        'title': 'Importación masiva de Personal (Background)',
    }
    return render(request, 'admin/mantenimiento/tecnicopuesto/import_background.html', context)

@staff_member_required
@csrf_exempt
def import_personal_process(request):
    """Triggers the Celery task for importing personal."""
    import sys
    ...
    if not is_confirm:
        ...
        print(f"[DEBUG] [Personal] Recibido archivo nuevo: {import_file.name}")
        file_ext = import_file.name.split('.')[-1].lower()
        temp_name = f'tmp/import_personal_{request.user.id}_{int(time.time())}.{file_ext}'
        ...
        print(f"[DEBUG] [Personal] Confirmando importación sobre archivo existente: {path}")
    
    # Limpiar cache de progreso anterior
    cache_key = f"import_personal_progress_{request.user.id}"
    ...
    print(f"[DEBUG] [Personal] POST: {request.POST}")
...
@staff_member_required
def import_personal_progress(request):
    """API to poll progress for personal import."""
