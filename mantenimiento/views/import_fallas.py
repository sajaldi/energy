...
from ..tasks import import_fallas_task
from django.views.decorators.csrf import csrf_exempt

@staff_member_required
def import_fallas_background(request):
    """Renders the upload form for background import."""
    from django.contrib import admin
    context = {
        **admin.site.each_context(request),
        'title': 'Importación masiva de Catálogo de Fallas (Background)',
    }
    return render(request, 'admin/mantenimiento/falla/import_background.html', context)

@staff_member_required
@csrf_exempt
def import_fallas_process(request):
    """Triggers the Celery task for importing failures."""
    import sys
    ...
    if not is_confirm:
        ...
        temp_name = f'tmp/import_fallas_{request.user.id}_{int(time.time())}.{file_ext}'
    ...
    cache_key = f"import_fallas_progress_{request.user.id}"
    ...
    import_name = request.POST.get('name') or f"Fallas: {import_file.name if import_file else os.path.basename(path)}"
    
    task = import_fallas_task.delay(
        path, 
        file_ext, 
...
@staff_member_required
def import_fallas_progress(request):
    """API to poll progress for failure catalog import."""
